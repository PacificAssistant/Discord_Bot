from sqlalchemy import create_engine, Column, Integer, String, Text, DateTime, ForeignKey,BigInteger,Date,ARRAY,Boolean,Table
...
class Audio(Base):
    ...
    file_id:Mapped[str] = mapped_column(String(255), nullable=True)  # ID файлу (наприклад, UUID)
    file_path:Mapped[str] = mapped_column(Text, nullable=False)  # Шлях до файлу (або URL)
    created_at:Mapped[date] = mapped_column(DateTime, default=date.today)
...
class User(Base):
    ...
    user_roles: Mapped[list["UserRoles"]] = relationship("UserRoles", back_populates="user", cascade="all, delete")

    #  Доступ до ролей через зв’язок many-to-many
    roles: Mapped[list["Roles"]] = relationship(
        "Roles",
        secondary="user_roles",
        viewonly=True  #  Не дозволяє змінювати напряму
    )
...
class Roles(Base):
    ...
    user_roles: Mapped[list["UserRoles"]] = relationship("UserRoles", back_populates="role", cascade="all, delete")
